...
roi_out = Martin_data.Detect_face(video_addr) #Martin的
# plt.ion()
plt.figure(figsize=(20,10))#會產生視窗所以要擺迴圈外
def OutputCSV():   
    Result = ('D:/dataset/datasetcsv/'+who+'_leftface.csv') #自錄的
    # Result = ('D:/dataset/datasetcsv/'+who_name+'_right_face.csv') #Martin的
    #Martin 0123:'right_face' , 4567:'left_face' , 891011:'nose'
...
while 1:
    ...
    Bh,Gh,Rh = cv2.split(vid[roi_out[4]:roi_out[5],roi_out[6]:roi_out[7]])#cv2.split:將圖片猜成三個通道  額頭
    # Bf,Gf,Rf = cv2.split(vid[284:324,818:888])
    # Bh,Gh,Rh = cv2.split(vid[400:500,700:850])#cv2.split:將圖片猜成三個通道
    # Bf,Gf,Rf = cv2.split(vid[roi_out[8]:roi_out[9],roi_out[10]:roi_out[11]])# 右臉頰
    # vid[roi_out[8]:roi_out[9],roi_out[10]:roi_out[11]]
    # vid[roi_out[12]:roi_out[13],roi_out[14]:roi_out[15]]
    ...
    # step6.peak prediction---------------------------------------------------
            # peakpredict(movingave(psealphalist_fir))    
        # --------------------------------產出CSV檔-----------------------------------------
            OutputCSV()
