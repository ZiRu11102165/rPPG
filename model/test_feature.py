...
for name in dirs:
    if os.path.splitext(name)[1] == ".csv":
        ...
        for row in range(80,len(data_x),1):   #sliding window *250*1
            Re_who.append(who)
...
def Model():
    ...
    model.add(Conv1D(filters=20,kernel_size=9,strides=1,input_shape=(80,1),padding="SAME",activation = 'gelu'))  # filters空間維度,kernel_size卷積窗口的長度,strides卷積的步長,
    ...
    model.add(Conv1D(filters=20,kernel_size=9,strides=1,padding="SAME",activation = 'gelu'))  # filters空間維度,kernel_size卷積窗口的長度,strides卷積的步長,
    model.add(BatchNormalization())
...
record = model.fit( X_train, Y_train,
                    ...
                    callbacks = [learning_rate_re,
                                 tf.keras.callbacks.EarlyStopping(monitor="val_loss", patience=5, mode='min', restore_best_weights=True,)],  #scheduler用
					)	

scores = model.evaluate(x=x_test,y=y_test,)
print(scores[0])

model.save('./keras_model_0222_feature.h5')
